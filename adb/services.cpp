...
void restart_root_service(int fd, void *cookie) {
    if (getuid() == 0) {
        ...
        char emotion_version[PROPERTY_VALUE_MAX];
        property_get("persist.sys.root_access", value, "0");
        property_get("ro.build.type", build_type, "");
        property_get("ro.emotion.version", emotion_version, "");

        if (strlen(emotion_version) > 0 && strcmp(build_type, "eng") != 0 && (atoi(value) & 2) != 2) {
